<template>
  <div class="general">
     {{$store.state.total.fossilFuel}}
      this is the general counter
      {{ $store.state.counter }} <button @click="$store.commit('decrement')">-</button>
      Round: {{round}}
      ...
      <div class="upgrades">
         <div v-for="(upgrade, index) in upgrades" :key="index" class="upgrade">
           ...
           {{ upgrade.amount }}
            ...
            <div class="details">
            <div class="cost">energyConsumption: {{ upgrade.energyConsumption }}</div>
            <div class="quantity">energyProduction: {{ upgrade.energyProduction }}</div>
            </div>
         </div>
      </div>
  </div>
